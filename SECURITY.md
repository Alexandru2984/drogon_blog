...
| 33| **Med**  | **Deleting your own comment deleted other people's replies.** `comments.parent_id` is `ON DELETE CASCADE` (0014), and `DELETE /comments/{id}` removed the row with a plain primary-key delete, so every reply underneath — whoever wrote it — went with it. Any account could erase a discussion by commenting, waiting for replies, and deleting the comment; the endpoint was reachable with an ordinary session and CSRF token even though the SPA never exposed it. Account erasure already worked around the cascade; the everyday delete path did not. Around it: a tombstoned comment could be revived by editing it, edits accepted empty content, and new replies (and their notifications) could still attach to a deleted comment. | `deleteComment` locks the row with `FOR UPDATE` — which conflicts with the `FOR KEY SHARE` a reply's foreign-key check takes, so no reply can land mid-decision — then, in a second statement whose snapshot is taken after the lock, tombstones the comment (`[deleted]`, `deleted_at`) if it has replies and deletes it only if it has none. Both run in one transaction on the auth worker pool, and the response waits for the commit. Edits and new replies refuse tombstoned comments, edits refuse empty content, and the comment list marks a tombstone `deleted: true` without naming its author. Regressions in `test/test_social.cc`. |

## Audit findings (September 2026)

A second systematic pass over the whole codebase, controller by controller and
helper by helper, with dynamic checks run against an isolated instance (never
production). Findings that required a change to the reverse proxy rather than
the application are tracked with the operator runbook, not here.

| #  | Severity | Issue | Fix |
|----|----------|-------|-----|
| 34 | **High** | **Per-IP rate limits were bypassable by changing path case.** Drogon routes registered handlers case-insensitively, but the rate limiter and the CSRF-exempt set compared `req->getPath()` against fixed lowercase strings with `==`. `GET /POSTS/SEARCH?q=…` reached the search handler while the exact match missed, so a bot looping the uppercase form evaded the per-IP FTS limiter entirely — the one control protecting the CPU-heavy `ts_headline` path. Confirmed on an isolated instance: 14/14 `200` for `/POSTS/SEARCH` versus `429`-after-burst for `/posts/search`. | The advice folds the path to lowercase once and matches the lowercase rule and exempt sets, so every case variant shares one bucket. Verified: `/POSTS/SEARCH` now `429`s after its burst. |
| 35 | **Med** | **Malformed input reached Drogon's top-level catch as an uncaught exception and became a bare `500` — which the access-log advice escalates to Sentry, so an anonymous caller could mint unbounded error events.** Three classes: an int path parameter is bound with `std::stoi` before the handler runs, so `/posts/abc` or an id past `INT_MAX` threw in the router (`/posts/{id}`, `/users/{id}`, `/posts/{id}/comments`, `/users/{id}/follow-stats`, … all `500`); a JSON body that parses but is not an object made `(*json)["field"]` throw `resolveReference`; a body nested past `json_parser_stack_limit` made `getJsonObject()` itself throw before the `!json` guard. All reproduced on an isolated instance. | `main.cc` installs an exception handler that answers `400` for `std::invalid_argument`/`std::out_of_range`/`Json::Exception` and keeps them out of the error budget; anything genuinely server-side stays a logged `500`. Verified: every case now returns `400`, valid requests still `404`/`401`, no `5xx` logged. |
| 36 | **Med** | **WebAuthn clone detection was defeatable.** `finishAuthentication` accepted any assertion whose `signCount` was 0 regardless of the stored counter, and the verify path's guard was `$1 = 0::bigint OR sign_count < $1`. An assertion reporting 0 against an authenticator whose counter had advanced was accepted *and* wrote `sign_count = 0`, resetting the counter and disabling regression detection for that credential permanently. | Both layers now accept only a strict advance, or 0 against a stored 0 (a genuinely non-incrementing authenticator). Verified against the exact guard SQL: clone-0-vs-stored-5 affects 0 rows and leaves the counter at 5; a real advance and a 0-vs-0 key still succeed; a same-value replay affects 0 rows. Regression in `test/test_2fa_management.cc`. |
| 37 | **Low** | **`${VAR}` values were pasted into `config.json` unescaped.** A database or SMTP password containing a `"` or `\` closed the JSON string early and the remainder was parsed as configuration — a startup failure, or with a crafted value an alteration of the effective config. | `expandEnv` JSON-escapes every substituted value (quotes, backslash, control chars) without adding surrounding quotes, so the numeric slots (`"port": ${DB_PORT}`) are unaffected. Verified: a value of `a"b\c<TAB>{}` now parses and the server boots. |
| 38 | **Low** | **The COSE key CBOR parser had memory-safety and correctness defects on attacker-supplied input** (the credential public key at registration): a CBOR negative integer decoded as `-(long long)n - 1` (signed-overflow UB for large `n`); `kty`/`alg`/`crv` narrowed to `int`, so a huge value could wrap onto a supported alg; and an unknown map value that was an array/map/tag left its body in the stream, desyncing every field after it. | Integers decode through a bounds-checked helper, `kty`/`alg`/`crv` are range-checked before narrowing, and unknown values are skipped with the reader's depth-capped `skip()`. A key that does not fit the expected shape is now rejected rather than misread. |
| 39 | **Low** | **The user-search query was a LIKE pattern, not a literal prefix.** `getAllUsers` built `Criteria(Like, q + "%")` from the raw query, so `?q=%` matched every user and `_` matched any character — the "start a conversation" picker doubled as a wildcard enumeration of the directory. | The term's `\ % _` are escaped (Postgres LIKE's default escape is backslash) so it matches literally. |
| 40 | **Low** | **Replaced avatars were never deleted.** Every profile-image upload wrote a fresh JPEG under `uploads/profiles/` and left the previous one on disk, so re-uploading leaked a file each time and the directory grew without bound. | After the row is updated, the superseded file is removed — confined to the profiles directory, skipped when unchanged, and refused if the path contains `..`. |
| 41 | **Low** | **`/.well-known/security.txt` advertised a Policy URL under a repository that does not exist** (`github.com/micutu/drogon_blog`), so a researcher following RFC 9116 hit a 404. | Points at the real repository (`github.com/Alexandru2984/drogon_blog`). |
| 42 | **Med** | **The `blog.micutu.com` vhost did not enforce the Cloudflare origin guard** that nearly every sibling vhost on the host carries. The origin (`127.0.0.1:8092` behind nginx) was reachable directly by anyone who resolved the origin IP — bypassing Cloudflare's WAF, rate limiting and DDoS edge — and the vhost also carried no `limit_req`/`limit_conn`. Combined with Drogon minting a 14-day in-memory session for every cookieless request (confirmed: `/healthz`, `/feed.xml`, `/posts` all emit `Set-Cookie`), a direct-to-origin flood could exhaust memory unthrottled. | The 443 server block now runs `if ($from_cloudflare_origin = 0) { return 403; }` (the `$from_cloudflare_origin` geo map already existed in `conf.d/cloudflare-origin-guard.conf`), refusing any peer that is not a published Cloudflare address; the port-80 ACME block is deliberately left open for renewals. Verified after reload: loopback and real Cloudflare traffic still `200`, `nginx -t` clean. Canonical copy in `ops/nginx/blog.micutu.com.conf`. Edge `limit_req`/`limit_conn` is the next layer and tracked separately. |
| 43 | **Low** | **An email that changed did not tell the previous address.** Changing the email already requires the current password (#7), but that is the pivot an attacker who has the password makes — retarget recovery to their own inbox — and the only place the real owner is still reachable, the old address, was told nothing while the verification mail went to the new one. | `updateProfile` sends a notice to the previous address once the change commits: what happened, and what to do if it was not them (reset the password, which revokes every session, and enable 2FA). Fire-and-forget through the existing mail queue. |
| 44 | **Low** | **Outgoing-email templates interpolated `username`/`SMTP_FROM_NAME` into HTML without escaping.** Not reachable today — usernames are `[A-Za-z0-9_-]` and the from-name is operator-set — but an email body is exactly where a future charset relaxation or a legacy row would become HTML injected into mail sent to other people. | `helpers/EmailHelper` escapes every interpolated field at the boundary. |
| 45 | **Low** | **libvips untrusted loaders were eligible on user uploads.** The avatar/post-image pipelines decode attacker-supplied files; libvips ships obscure, less-audited "untrusted" format loaders that were not blocked, so a malicious file could select a rarely-exercised decoder. | `image::initLibrary` calls `vips_block_untrusted_set(TRUE)`, so only mainstream JPEG/PNG/WebP/GIF loaders (already the sniffer's allowlist) can touch user input. Defence-in-depth behind the magic-byte sniff and the systemd sandbox. |
...
- **Database schema (FKs, cascades, upsert constraints).** Every foreign key declares an explicit `ON DELETE`: user-owned rows `CASCADE`, while `audit_log.actor_id`, `reports.reporter_id`/`resolved_by` and the `hidden_by` columns are `SET NULL` so an account deletion cannot erase an audit, report or moderation trail. The one cross-user cascade, `comments.parent_id`, is handled in code by tombstoning (#33). Every `ON CONFLICT` in the code is backed by a matching constraint — `likes UNIQUE(post_id,user_id)`, the `bookmarks`/`follows`/`post_tags`/`post_views` primary keys, `tags.slug`, `user_totp_secrets.user_id`, and the partial `idx_reports_one_open_per_reporter` — so the idempotent upserts cannot silently degrade to duplicate rows or `500`s.
